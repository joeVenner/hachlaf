...
## [2026-07-10] Domaines Pinned Composition — Restored Three Cards and Premium Title
- **What changed:** Fixed regressions in the pinned `DomainesSection` where only the first card appeared and the section title was too small. Restored the large premium title, ensured all three cards are always mounted in the DOM, and rebuilt the stacking timeline with explicit phases mapped to a 10-unit duration: 0–15% hold Card 1, 15–42% Card 2 stacks over Card 1, 42–48% hold Card 2, 48–75% Card 3 stacks over Card 2, 75–90% hold the complete stack, 90–100% release. Added `ScrollTrigger.refresh()` after stage images load. Updated CSS header and stage dimensions: `.domains-header` is `90vw` with `175px` max height and `28px` vertical padding; `.domains-stage` is `clamp(500px, 64vh, 700px)`.
- **Why:** The previous pinned implementation rendered three cards in the DOM but used a compressed timeline that made Cards 2 and 3 hard to perceive, and the title utility had been shrunk too much during earlier iterations. A clearer phased timeline with explicit hold states and restored premium typography gives the intended “title fixed + three cards visibly stack + hold + release” experience.
- **Impact:** `src/components/DomainesSection.jsx`, `src/index.css`, `CLAUDE.md`.
- **Edge case:** All three desktop cards exist in the DOM even when the mobile fallback is visible; they are simply hidden by the parent `.domains-pin { display: none }` below 768px. The image-load refresh only fires for images inside `.domains-stage`; if an image fails to load it still counts toward the refresh counter via the `error` listener so ScrollTrigger does not wait indefinitely.

## [2026-07-10] Domaines Visual Polish — Remove Gray Strip and Restore Heading Font
- **What changed:** Fixed two visual regressions in the pinned `DomainesSection`. Removed the gray/white strip at the top edge of the cards by setting `.domains-stage` and `.domain-card` to `background: transparent`, replacing the `#fff` card background, and adding a `-2px` overscan to the image container (`-inset-[2px]`) so subpixel transform gaps show only image pixels. Reduced final stacked offsets to a subtle ~4–8px edge (`yPercent: -4` then `-8` for Card 1; `yPercent: -4` for Card 2; Card 3 at `0`) with Card 1 scaling to `0.95` only at the very end. Restored the section title to use the site’s display font token (`font-family: var(--font-display)`), bumped size to `clamp(64px, 6.2vw, 108px)`, set `letter-spacing: -0.045em`, and added `white-space: nowrap` for viewports ≥1200px.
- **Why:** The gray strip was caused by the card/stage defaulting to a white background and by the previous stacked offsets exposing that background behind scaled cards. The title had drifted away from the site’s actual heading font/size during iterative clamp adjustments, making it look like default body text instead of a premium section heading.
- **Impact:** `src/components/DomainesSection.jsx`, `src/index.css`, `CLAUDE.md`.
- **Edge case:** The image overscan intentionally extends 2px outside the card on every side; because the image uses `object-fit: cover` and the card uses `overflow: hidden`, no part of the overscan is visible beyond the card boundary. The reduced stacked offsets mean the layered edge is only a few pixels tall.

## [2026-07-10] Premium Contact Section and Footer Redesign
- **What changed:** Redesigned `CTASection` and `Footer` to close the page with the same premium quality as the Hero and Domaines sections. The contact area now uses a dark navy gradient (`#163A78` → `#0F2F66` → `#0A244D`) with a 3% blueprint-grid texture, a two-column `max-w-[1380px]` layout (45% introduction + 55% glass form), and a form panel with `rgba(255,255,255,0.05)`, `backdrop-filter: blur(18px)` and a thin white border. Added new fields (name, company, email, phone, project type, budget, message), a gold `Request a Quote` CTA, an outline-white `Contact via Email` CTA, and a discreet WhatsApp text link in place of the previous bright green button. The footer now sits on a warm gray `#F8F9FB` background with a full-width closing statement (`Building confidence. / Delivering excellence.`), a thin animated gold gradient line, a clean 4-column grid and a minimal bottom bar. New CSS utilities were added: `.bg-contact-gradient`, `.glass-form`, `.premium-input`, `.premium-label`, `.premium-select`, `.btn-outline-white`, `.footer-heading`.
- **Why:** The previous contact section felt like a generic template with a flat navy background and centered form, and the footer used a common three-column layout. Aligning the bottom of the page with the cinematic hero and stacked parallax sections creates a cohesive, high-end brand experience in the style of Vinci, Bechtel or Skanska.
- **Impact:** `src/components/CTASection.jsx`, `src/components/Footer.jsx`, `src/data/content.js`, `src/index.css`, `CLAUDE.md`.
- **Edge case:** `CTASection` now expects the expanded `contact` data model (`eyebrowNew`, `titleNew`, `paragraph`, `emailLabel`, `phoneLabel`, `projectTypeLabel`, `budgetLabel`, `quoteButton`, `emailButton`, `whatsappLinkText`, `projectTypes`, `budgets`, etc.) in both locales. The old `cta` prop is still passed by `HomePage` but is no longer used inside the component. Native `<select>` elements inside the glass form inherit a dark option background so the dropdown remains readable on the navy gradient.
