...
- **What changed:** Fixed regressions in the pinned `DomainesSection` where only the first card appeared and the section title was too small. Restored the large premium title with `.domains-title` set to `clamp(58px, 5.8vw, 96px)`, weight 800, letter-spacing `-0.035em`, and added a dedicated `.domains-eyebrow` utility for the cyan uppercase label. Ensured all three cards are always mounted in the DOM inside `.domains-stage` with initial `opacity: 1` and `visibility: visible`; Card 2 and Card 3 start below the stage (`yPercent: 105` and `110`) and slide up during the pinned scroll timeline. Rebuilt the stacking timeline with explicit phases mapped to a 10-unit duration: 0–15% hold Card 1, 15–42% Card 2 stacks over Card 1, 42–48% hold Card 2, 48–75% Card 3 stacks over Card 2, 75–90% hold the complete stack, 90–100% release. Final layered offsets are `yPercent: -9` / `scale: 0.95` for Card 1 and `yPercent: -2.25` / `scale: 0.975` for Card 2, with Card 3 at `scale: 1`. Added `ScrollTrigger.refresh()` after stage images load. Updated CSS header and stage dimensions: `.domains-header` is `90vw` with `175px` max height and `28px` vertical padding; `.domains-stage` is `clamp(500px, 64vh, 700px)`.
